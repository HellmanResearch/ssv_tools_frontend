...
master_api.interceptors.request.use(config => {
...
master_api.interceptors.response.use(response => {
    store.commit("reduce_request_count")
    return response
}, error => {
    store.commit("reduce_request_count")
    ...
    if (error.response.status == 401) {
        if (error.config.url == "user/users/get-my-info") {
